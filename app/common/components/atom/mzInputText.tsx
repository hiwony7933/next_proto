...
import searchIcon from "@/public/icons/icon_32_search.svg";
import clearIcon from "@/public/icons/icon_20_close_black.svg";
...
export interface MzInputTextProps
  ...
  search?: boolean; // 검색 버튼 노출 여부(boolean)
...
const MzInputText = forwardRef<HTMLInputElement, MzInputTextProps>(
  (
    ...
    const statusClass = search
      ? S.isSearch
      : status
        ? S[`is${status.charAt(0).toUpperCase()}${status.slice(1)}`]
        : undefined;
    const wrapperClassName = classnames(S.mzInputText, statusClass, className);
          ...
          <div className={S.mzInputText__inputWrapper__actions}>
            {hasValue && isInteractive && (
              <button
                ...
                onClick={handleClear}
              >
                <Image src={clearIcon} alt="" width={20} height={20} />
              </button>
            )}
            {search && (
              <button
                ...
                disabled={rest.disabled === true || !onSearch}
              >
