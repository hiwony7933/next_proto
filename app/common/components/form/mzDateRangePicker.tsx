...
export const MzDateRangePicker = ({
  ...
  const handleCompleteInput = (inputValue: string, type: 'from' | 'to') => {
    ...
    if (masked.length === 10) {
      ...
      const newValue: [string, string] = type === 'from' ? ['', value[1]] : [value[0], ''];
      onChange(newValue);
    }
  };

  // 입력 핸들러 (개선된 버전)
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>, type: 'from' | 'to') => {
  ...
  const handleCalendarChange = (date: Date | null, type: 'from' | 'to') => {
    ...
    const newValue: [string, string] = type === 'from' ? [dateStr, value[1]] : [value[0], dateStr];
