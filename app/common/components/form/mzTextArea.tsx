...
export default function MzTextArea({
      ...
      <div className={`${className} ${styles.mzTextArea}`} >
        ...
        {count === 'YES' && (
          <div className={`${styles.byteCount}`}>
            <em className={byte < (minBite || 0) ? "" : `${styles.red}`}>{byte} byte</em>{maxBite ? ` / ${maxBite} byte` : ''}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className={`${className} ${styles.mzTextArea}`} >
      ...
      />
      {count === 'YES' && (
        <div className={`${styles.byteCount} ${byte < (minBite || 0) ? 'red' : undefined}`}>
          <em className={byte < (minBite || 0) ? "" : `${styles.red}`}>{byte} byte</em>{maxBite ? ` / ${maxBite} byte` : ''}
        </div>
      )}
    </div>
